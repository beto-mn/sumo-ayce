...
describe('loyaltyConfig', () => {
  ...
  it('falls back to 10 when LOYALTY_POINTS_PER_VISIT is 0', async () => {
  ...
  it('falls back to 10 when LOYALTY_POINTS_PER_VISIT is negative', async () => {
  ...
  it('reads velocityThreshold from env', async () => {
    process.env.LOYALTY_VELOCITY_THRESHOLD = '10'
    const { loyaltyConfig } = await import(
      '../../../server/utils/loyalty-config'
    )
    expect(loyaltyConfig.velocityThreshold).toBe(10)
  })
