...
| `tyc_es` / `tyc_en` | string (optional) | **Feature 027, Part A.** Bilingual Terms & Conditions text, shown on the promo card's back face when the visitor clicks/taps it (flip-to-terms). Assumed field keys, following this CPT's `_es`/`_en` naming convention — pending confirmation from a live payload (WP admin does not surface the underlying ACF field key). **Bilingual-completeness rule**: the flip affordance is offered ONLY when BOTH `tyc_es` AND `tyc_en` are present and non-empty; a promo with only one language filled in is treated identically to a promo with neither (no same-language fallback, unlike `badge_en`'s fallback to `badge_es`). **Coordination status (as of this feature)**: the client is actively wiring these two textarea fields into the `promociones` CPT edit screen in WP admin (screenshot: `specs/027-promo-flip-menu-card-tweaks/assets/source/wp-admin-tyc-fields.png`) — not yet confirmed live with real content. This repo's parsing/rendering is defensive regardless: any promo with an absent, empty, or partially-filled `tyc_*` pair simply shows no flip affordance (never an error, never an empty back face). |
