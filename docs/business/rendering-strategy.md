...
| 012 Branches page                  | `/sucursales`     | `isr: 3600`             | WP CPT `sucursales`. Geolocation + map (provider-agnostic, default Mapbox — see [`maps-strategy.md`](./maps-strategy.md)) + distance happen client-side over cached list. |
| 013 Reservation modal              | (overlay)         | n/a                     | Renders on top of any page. Form posts to `/api/v1/reservations` (dynamic Vercel function). |
| 014 Reservation page               | `/reserve`        | `ssr: true`             | Branch list fetched server-side on every request. No ISR — reservations are transactional. |
| 015 Loyalty user portal            | `/lealtad`        | `ssr: true`             | Per-user balance + history. Always SSR for fresh data + SEO-safe shell.                    |
